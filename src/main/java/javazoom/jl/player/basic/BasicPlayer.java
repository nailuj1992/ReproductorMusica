...
public class BasicPlayer implements BasicController, Runnable {
    ...
    protected void stopPlayback() {
        if ((m_status == PLAYING) || (m_status == PAUSED)) {
            m_status = STOPPED;
//        	try {
//				m_thread.join(1000); // wait at most one second for thread to finish
//			} catch (InterruptedException e) {
//			}
            synchronized (m_line_lock) {
                if (m_line != null) {
...
//					m_line.flush();
    ...
    protected void startPlayback() throws BasicPlayerException {
        ...
        if (m_status == OPENED) {
            ...
            if (m_thread != null && m_thread.isAlive()) // before v3.1 -> (!(m_thread == null || !m_thread.isAlive()))
            {
        ...
        synchronized (m_audioInputStream) {
            ...
            while ((nBytesRead != -1) && (m_status != STOPPED) && (m_status != SEEKING) && (m_status != UNKNOWN)) {
                if (m_status == PLAYING) {
                    ...
                    // Pause
                    try {
                        // before v3.1 slept 1sec (way too long)
                        // resuming now only takes 70ms (14 times faster)
                        Thread.sleep(70);
                    } catch (InterruptedException e) {
                        log.error("Thread cannot sleep(70)", e);
                    }
                }
            }
    ...
    /**
     * Returns the current thread that plays media (may be finished) <br>
     *
     * @return probably null if play() has not been called yet
     */
